...
pub async fn get_persoana_fizica(path: web::Path<Uuid>, pool: web::Data<PgPool>) -> HttpResponse {
...
pub async fn create_persoana_fizica(
    payload: web::Json<PersoanaFizicaRequest>,
    ...
    let mut tx = match pool.begin().await {
        ...
        Err(err) => {
            ...
            return HttpResponse::InternalServerError().body("Eroare la pornirea tranzacției");
        }
    };
    ...
    // 2) Inserăm adresa în tabela address
    // presupunem schema:
    ...
    match result {
        Ok(_) => {
            if let Err(err) = tx.commit().await {
                ...
                return HttpResponse::InternalServerError().body("Eroare la commit");
            }
            HttpResponse::Created().json(new_persoana_fizica)
        }
        Err(sqlx::Error::Database(db_err))
            if db_err.constraint() == Some("persoana_fizica_cnp_key") =>
        {
            let _ = tx.rollback().await;
            HttpResponse::Conflict().body("CNP-ul există deja în sistem")
        }
        Err(err) => {
            eprintln!("Database error inserting persoana_fizica: {:?}", err);
            let _ = tx.rollback().await;
            HttpResponse::InternalServerError().body("Eroare la salvarea persoanei fizice")
        }
    }
}

...
impl InvoiceService {
    ...
    pub async fn update_invoice(&self, id: Uuid, request: CreateInvoiceRequest) -> Result<InvoiceResponse> {
        let mut store = INVOICE_STORE.lock().unwrap();
    ...
    pub async fn list_invoices(&self) -> Result<Vec<InvoiceResponse>> {
        ...
        let invoices: Vec<InvoiceResponse> = store.values()
    ...
    pub async fn validate_invoice(&self, id: Uuid) -> Result<serde_json::Value> {
        ...
        let is_valid = !invoice.numar_serie.is_empty() 
            && invoice.total_de_plata > 0.0;
